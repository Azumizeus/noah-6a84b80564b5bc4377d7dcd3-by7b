...
<<<<<<< HEAD
  { category: 'Design & Créatif', roles: [{ id: 'uxui', label: '🎨 UX/UI Designer', weight: 10 }, { id: 'artist', label: '🖌️ Artist / 3D', weight: 8 }, { id: 'motion', label: '🎬 Motion Designer', weight: 6 }] },
  ...
  { category: 'Capital', roles: [{ id: 'investor', label: '💰 Investisseur', weight: 0 }, { id: 'advisor', label: '🧭 Advisor', weight: 3 }] },
];
export const ALL_ROLES = ROLE_GROUPS.flatMap(g => g.roles);
const SHORT: Record<string, string> = { founder: 'Founder', cofounder: 'CoFounder', pm: 'PM', leaddev: 'Lead Dev', rustdev: 'Rust Dev', frontend: 'Frontend', backend: 'Backend', fullstack: 'Fullstack', mobile: 'Mobile', gamedev: 'Game Dev', devops: 'DevOps', security: 'Security', uxui: 'UX/UI', artist: 'Artist', motion: 'Motion', tokenomics: 'Tokenomics', marketing: 'Marketing', community: 'Community', growth: 'Growth', content: 'Content', legal: 'Legal', investor: 'Investor', advisor: 'Advisor' };
...
export function slugId(name: string): string { const base = name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9-]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 16).replace(/-$/g, ''); return `${base}-${Math.random().toString(36).slice(2, 6)}`; }
=======
