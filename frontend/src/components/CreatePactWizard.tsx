...
import { createProject, addMember, approve, finalize, getProgram } from '../lib/anchor';
...
export function CreatePactWizard({ onSuccess }: Props) {
  ...
  if (!publicKey) return <p className="text-ink-300">Connectez votre wallet</p>;
  ...
  const handleAddMember = async () => {
    if (!projectPda || members.length === 0) return;
    setLoading(true);
    try {
