...
### Session 30/08 (nuit, suite) — ce qu'il faut savoir avant de continuer

**`anchor-spl` importée pour la première fois.** La dépendance dormait dans
`Cargo.toml` depuis longtemps (jamais liée au binaire faute d'import). Le
30/08, `fund_spl`/`distribute_spl` l'importent enfin pour le financement
multi-token (Option B du plan, GO explicite utilisateur). **Ce code n'a
jamais été compilé** : aucun `cargo`/`anchor` dans le sandbox Cowork. Ne
pas le traiter comme validé avant un `anchor build` réel.

**Le mint USDT communiqué par l'utilisateur est suspect.** Il correspond
mot pour mot au mint déjà documenté comme USDC dans `tokenBalances.ts`.
Vraisemblablement un copier-coller de la mauvaise valeur — à reconfirmer
avant d'écrire quoi que ce soit côté frontend avec cette adresse.

**FreeGrid, piège à ne pas retomber dedans** : `onLayoutChange(layout,
layouts)` de react-grid-layout — `layouts` (le plan par breakpoint) n'est
à jour qu'à la FIN d'un geste. L'utiliser pour l'état React pendant un
drag actif bloque le glissé. Toujours dériver l'affichage du `layout`
courant ; ne conditionner que la PERSISTANCE au breakpoint actif.

**Console admin/dev demandée** — fonctions sensibles (ban, don d'XP
arbitraire, logs système). Voir `ADMIN_CONSOLE_PLAN.md` : accès caché par
route non listée n'est PAS une protection en soi (sécurité par
l'obscurité) — doit être doublé d'un contrôle de rôle serveur (RLS) et
idéalement d'un 2FA sur les comptes concernés. Ne pas construire l'UI
avant que ce modèle soit validé.
